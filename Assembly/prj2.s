...
main:           la $sp, stack           ! Initialize stack pointer
		...
		la $a1, ti_inthandler
		sw $a1, 1($a0) 		! store address of handler in IVT
...
ti_inthandler:
		...
		! Device Code
		la $s0, seconds
		lw $s0, 0($s0)		! load seconds address in $s0

		lw $a0, 0($s0)		! load seconds into $a0 
		addi $a0, $a0, 1	! increment seconds (line: 100)
		sw $a0, 0($s0)		! store new seconds

		addi $a1, $zero, 60	
		beq $a0, $a1, min	! if (seconds == 60)
		beq $zero, $zero, fin	! else, I'm outta here

min:		sw $zero, 0($s0)	! reset seconds		

		la $s0, minutes
		lw $s0, 0($s0)		! load minutes address in $s0

		lw $a0, 0($s0)		! load minutes into $a0
		addi $a0, $a0, 1	! increment minutes
		sw $a0, 0($s0)		! store new minutes

		addi $a1, $zero, 60	
		beq $a0, $a1, hour	! if (minutes == 60)
		beq $zero, $zero, fin	! else, I'm outta here

hour:		sw $zero, 0($s0)	! reset minutes		
		
		la $s0, hours
		lw $s0, 0($s0)		! load hours address in $s0

		lw $a0, 0($s0)		! load hours into $a0
		addi $a0, $a0, 1	! increment hours
		sw $a0, 0($s0)		! store new hours

		addi $a1, $zero, 24	
		beq $a0, $a1, reset	! if (hours == 24)
		beq $zero, $zero, fin	! else, I'm outta here

reset:		sw $zero, 0($s0)	! reset hours	
